...
- **Status**: Proposed. **The packet type and codecs are implemented** (`crates/latentmesh-core`, `crates/latentmesh-align`); the network transport is not. Updated 2026-08-28 (affine projection contract, see Decision bullet 3).
- **Date**: 2026-08-18
...
- **Alignment (`latentmesh-align`) is the general, well-defined technique — orthogonal Procrustes via SVD** (`R = argmin_{R orthogonal} ||A R − B||_F`, closed-form as `R = U Vᵀ` from the SVD of `Aᵀ B`), *not* a re-implementation of StateBridge's specific method. This crate makes an honest claim: given calibration pairs `(a_i, b_i)` in the two spaces, it recovers the least-squares-optimal orthogonal alignment, verifiably, on synthetic data. It does **not** claim StateBridge's reported 22/26 best-or-tied-best result — that requires live heterogeneous LLM hidden states, which this repo does not have access to (ADR-001 §8). `confidence` is derived from the residual `||A R̂ − B||_F` on the calibration set, scaled to `[0,1]` — a real, computed number, not an LLM-asserted one.
  ...
  **Updated 2026-08-28 (live-experiment patch, `docs/research/024` §3):** the projection contract is now **affine**: `aligned(z) = μ_r + α · (z − μ_s) · R`, fitted by `AlignmentTransform::fit_affine` (mean-center each side, then the same Procrustes solvers). `μ_s`/`μ_r` are carried **in the hashed transform struct**, so `transform_hash` binds the full affine map — two transforms differing only in centering hash differently. The plain `fit` path is unchanged (empty means ⇒ `α · z · R` exactly as before). Two cost fixes with no semantic change: the projection `DMatrix` is built once per fit and cached (`apply` previously rebuilt it — ~25 MB of allocation per call at 2048×1536), and `content_hash` is computed once per fit and cached (previously re-serialized the matrix per call). A named `MᵀM`-eigendecomposition fallback for the dense rectangular SVD sits behind the same fit API (`procrustes_mtm.rs`); measured 2026-08-28 at the run-1 shape (2048×1536, release build, two runs): dense SVD **16.48 s / 17.65 s**, `MᵀM`-eig **2.17 s / 3.67 s**, max entrywise difference 1e-6 — dense passes the design's <5 min S1b gate with ~17× margin and remains the default; the fallback stays the named alternative.
...
- `crates/latentmesh-align/src/lib.rs` — `AlignmentTransform::fit(pairs) -> (R, α, confidence)` via SVD-based orthogonal Procrustes; `fit_affine(pairs)` additionally storing `(μ_s, μ_r)` (2026-08-28); `apply(z) -> aligned z` via the cached projection matrix. Tests: recovers a known random orthogonal `R` from noiseless synthetic pairs; degrades gracefully (lower confidence) under injected noise; `apply` composed with the known inverse recovers the original vector within tolerance; `fit_affine` recovers a known affine map plain `fit` cannot; hashes/apply stable across serde round trips; `MᵀM`-eig fallback matches the dense solver on full-rank rectangular problems.
