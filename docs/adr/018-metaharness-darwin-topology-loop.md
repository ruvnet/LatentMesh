...
- **Status**: Accepted and implemented (deterministic simulation with signed receipts; live multi-agent deployment not claimed).
- **Date**: 2026-08-26
...
- **`Topology`** — `(agents, edges)` with per-edge `Encoding` and measured
  `ΔV`; `Mutation ∈ {AddEdge, RemoveEdge, ReweightEdge, ChangeEncoding,
  DeactivateAgent, ReactivateAgent}`. Mutations are proposals; a mutation that
  would raise any edge's authority above its constitution cap is rejected
  before evaluation (`authority never expands` is enforced in the mutation
  applier, not the fitness function).
- **Fitness is ADR-006's objective, with causal Quality**: an edge contributes
  to `Quality(G)` only if its ΔV survives `latentmesh-gate`'s four-control
  causal test in the evaluation environment; an unverified edge contributes
  zero regardless of apparent correlation. Governance-mandatory edges are
  never auto-removed even at ΔV ≈ 0, per ADR-006's control-vs-cognition
  distinction.
- **The loop is deterministic**: seeded RNG, frozen task environment
  ...
  optimizer), generation-by-generation evaluation, elitist selection.
...
- **MetaHarness receipts**: each run emits a JSON receipt — seed, environment
  hash, generations, compute proxy before/after, surviving edges with their
  control-test results, and an explicit evidence label ("deterministic
  simulation; not a live multi-agent deployment") — in the same spirit as
  `harness/air`'s stage-separated benchmark receipt. A new `harness/evolve`
...
- ADR-006's acceptance test is now executable: the shipped environment checks
  ...
  the four-control test. Passing it proves the loop optimizes the causal
