...
- **Fixed `frame_mtu = 227`, not Air's native 256-byte `FRAME_MAX_BYTES`.**
  **Corrected 2026-08-27 by live interop against real firmware** (this ADR
  originally said 233): mesh.proto's `DATA_PAYLOAD_LEN = 233` bounds the
  *encoded `Data` submessage*, so the raw payload loses ~6 bytes of protobuf
  field overhead (portnum tag/varint + payload tag/length). A boundary search
  against meshtasticd 2.7.26 (portduino, TCP device API) measured: ≤227-byte
  payloads round-trip reliably; 228-231 transmit but echo unreliably; 232+ are
  rejected outright with `Routing.Error.TOO_LARGE` (`Error=7, return NAK and
  drop packet`). The interop suite keeps a 232-byte tripwire scenario so a
  future firmware raising the ceiling is detected rather than assumed.
  Meshtastic still does **not** auto-fragment application payloads (three
  converging primary-source signals, research doc §1.3; `ChunkedPayload` in
  `mesh.proto:2997-3049` is a distinct admin-surface mechanism). So Air's own
  16-byte frame overhead (`FRAME_HEADER_BYTES=12` + 4-byte CRC32C) comes out
  of the 227-byte budget: `227 − 16 = 211` usable Air-fragment payload bytes
  per Meshtastic packet. The adapter must call
  `fragment_message(meta, message, 227)` — a named constant
  (`MESHTASTIC_FRAME_MTU: usize = 227`), never Air's default MTU, or frames
  silently overflow the firmware's acceptance ceiling and get NAK'd.
  - Verified against source: LMS1 header is 48 bytes
    ...
    slots, several updates under one signature) crosses 211 bytes and needs
    ...
    (32 × 211 = 6,752 bytes effective, vs. 32 × 240 = 7,680 bytes native).
...
- **Device API framing**: `0x94 0xc3` (`START1`/`START2`) + 2-byte
  ...
  serial or TCP to a host-side Meshtastic node (originally graded a
  secondary-source claim; **since verified 2026-08-27 byte-for-byte against
  real firmware** — the full `want_config_id` handshake decodes meshtasticd
  2.7.26's actual FromRadio stream with this framing). This is the adapter's actual implementation
...
itself. In exchange, LatentMesh Air gives up MTU headroom: 211 usable bytes
...
| Claim | Status |
|---|---|
| `frame_mtu = 227` fragmentation and `Reassembler` round-trip | **Validated against real Meshtastic firmware** (2026-08-27, meshtasticd 2.7.26, portduino, simulated radio, TCP device API): single-fragment and 2-fragment broadcasts route through the firmware and reassemble byte-identical; a 232-byte tripwire confirms the `TOO_LARGE` ceiling each run. Opt-in via `MESHTASTICD_ADDR` (`examples/meshtasticd_interop.rs`, `tests/live_meshtasticd.rs`) |
| `0x94 0xc3` device-API frame encode/decode | **Validated against real firmware** — full `want_config_id` handshake decodes meshtasticd 2.7.26's live FromRadio stream |
...
| Self-addressed unicast (`to = own node num`) echoed back via FromRadio | **Empirically false** on 2.7.26 — firmware returns a ROUTING_APP NAK instead; only broadcast routes back to the API client. Interop suites must use broadcast |
| Broadcast self-echo mechanism | **Portduino simulator quirk** — the sim radio re-wraps echoed traffic as `SIMULATOR_APP(69)`; not expected against a genuine second peer on real hardware |
| Real Meshtastic node connection over serial, or two physical peers | **Hardware-pending** — no Meshtastic LoRa hardware present on this host; software interop above is one evidence rung below this |
...
`meshtastic/meshtasticd` Linux build (portduino, simulated radio) over its
TCP device API. This produced the `frame_mtu` correction above (233 → 227,
usable 217 → 211, applied in lock-step to Rust, the C ABI defaults, and the
`harness/integration` acceptance bounds/optimizer space) and two behavioral
findings recorded in the boundary table (unicast no-echo; `SIMULATOR_APP`
echo quirk). Evidence label: real firmware over TCP — **no RF, no
over-the-air claim**. Entry into ADR-014's
