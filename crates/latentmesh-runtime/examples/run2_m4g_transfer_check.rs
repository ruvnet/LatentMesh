//! Run-2 M4g — registered transfer check (ADR-024 M4c/M4d caveat mitigation,
//! inherited), run BETWEEN training and the frozen probe, gating the probe.
//!
//! This is **byte-for-byte `run2_m4d_transfer_check.rs`** with two
//! substitutions: M4g's artifact paths, and the injection operator
//! (`InjectionMode::Fuse` — `h[slot] += c*v` — instead of
//! `InjectionMode::Overwrite`). Deliberately: the three rungs'
//! `mean_fused_nll_{init,trained}` numbers are only comparable if the
//! measurement is literally the same one, and the operator is exactly the
//! factor M4g registers.
//!
//! The M4g adapter was trained through the COMPOSED differentiable BF16
//! forward (`latentmesh-train::qwen2_c`); the frozen probe runs the vendored
//! FUSED BF16 forward. (Note the unfortunate collision of the word "fused":
//! "fused forward" = candle's fused inference kernels, unchanged since M4c;
//! "fuse operator" = M4g's residual add. They are unrelated.) The measured
//! numeric gap between the two forwards (116/128 argmax agreement, max
//! |dlogit| 8.19 at L=128) means a probe null could be confounded unless the
//! training improvement is first shown to TRANSFER across it. This check does
//! exactly that, inference-only, with NO probe items, NO generation, NO
//! frozen-probe draw.
//!
//! Frozen pass criterion (quoted from the training receipt, frozen there
//! BEFORE this check ran): mean fused-forward NLL(trained) < mean fused-forward
//! NLL(init); per-item wins/losses + sign test reported as secondary, not
//! gating. On fail: the frozen probe is NOT invoked (run2_m4g_probe refuses).
//!
//! Run: PATH=/usr/local/cuda-12.8/bin:$PATH \
//!      cargo run --release --features cuda --example run2_m4g_transfer_check
...
/// Frozen span rule — MUST match `train_m4g_fuse.rs` (`SEQ_CAP`,
...
/// **THE ONE CHANGED FACTOR** — pinned to the same value the M4g trainer and
/// the M4g probe use, and asserted against the training receipt below.
...
fn main() -> anyhow::Result<()> {
    ...
    let train_mode = tr["training"]["injection_operator"]["mode"]
        .as_str()
        .expect("training receipt training.injection_operator.mode");
    anyhow::ensure!(
        train_mode == INJECT_MODE.tag(),
        "training receipt used injection operator '{train_mode}' but this check uses '{}' — \
         the transfer measurement must run through the SAME operator the adapter was trained for",
        INJECT_MODE.tag()
    );
    println!(
        "injection operator: {} ({})",
    ...
    let (gn, gmax, _) = common::mlp::verify_against_golden(
        &trained,
        &crate_path("receipts/run2-m4g-golden-mlp-fuse-cellL18toL14.json"),
        common::m3::GOLDEN_REL_TOL,
    )?;
    let init = MlpTransform::load(&crate_path(
        "receipts/run2-m4g-mlp-fuse-init-cellL18toL14.f32bin",
    ))?;
    anyhow::ensure!(
    ...
    let (gni, gmaxi, _) = common::mlp::verify_against_golden(
        &init,
        &crate_path("receipts/run2-m4g-golden-mlp-fuse-init-cellL18toL14.json"),
        common::m3::GOLDEN_REL_TOL,
    )?;
    ...
    let receipt = serde_json::json!({
        "stage": "run2-m4g-transfer-check",
        "design": "ADR-024 M4g (§ 'M4g PRE-REGISTRATION — fuse instead of overwrite'); the M4c/M4d registered caveat mitigation inherited unchanged, criterion frozen in the training receipt BEFORE this check ran; inference-only, no probe items, no generation, no frozen-probe draw",
        "injection_operator": {"mode": INJECT_MODE.tag(), "equation": INJECT_MODE.equation(),
            "asserted_equal_to_training_receipt": true,
            "prior_rungs": "overwrite (M3, M4, M4c, M4d)"},
        ...
        "config": {
            ...
            "delivery": "hand-rolled MLP per-token over the item's FULL generated-span dump rows -> mean-pool TRANSLATED rows -> rescale to VENDORED natural inject-block median -> 8-slot RESIDUAL ADD, h[slot] += c*v (probe delivery path exactly, M4g operator)",
        ...
        "gate_pass": pass,
        "verdict": if pass {
            "improvement TRANSFERS across the composed->fused BF16 gap under the FUSE operator; the frozen probe may run"
        } else {
            "improvement does NOT transfer — the frozen probe must NOT be invoked (a null would be confounded by the numeric gap); this receipt is the honest M4g outcome for this branch"
        },
