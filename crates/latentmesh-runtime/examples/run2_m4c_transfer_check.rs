//! Run-2 M4c — registered transfer check (ADR-024 M4c caveat mitigation),
//! run BETWEEN training and the frozen probe, gating the probe.
//!
//! The M4c adapter was trained through the COMPOSED differentiable BF16
...
//! On fail: the frozen probe is NOT invoked (run2_m4c_probe refuses to run).
//!
//! Run: PATH=/usr/local/cuda-12.8/bin:$PATH \
//!      cargo run --release --features cuda --example run2_m4c_transfer_check
...
/// Frozen span rule — MUST match `train_m4c_taskloss.rs` (`SEQ_CAP`,
...
fn main() -> anyhow::Result<()> {
    ...
    let (gn, gmax, _) = common::mlp::verify_against_golden(
        &trained,
        &crate_path("receipts/run2-m4c-golden-mlp-taskloss-cellL18toL14.json"),
    ...
    let (gni, gmaxi, _) = common::mlp::verify_against_golden(
        &init,
        &crate_path("receipts/run2-m4c-golden-mlp-taskloss-init-cellL18toL14.json"),
        common::m3::GOLDEN_REL_TOL,
    )?;
    ...
    let receipt = serde_json::json!({
        "stage": "run2-m4c-transfer-check",
        "design": "ADR-024 M4c registered caveat mitigation, criterion frozen in the training receipt BEFORE this check ran; inference-only, no probe items, no generation, no frozen-probe draw",
        ...
        "verdict": if pass {
            ...
            "improvement does NOT transfer — the frozen probe must NOT be invoked (a null would be confounded by the numeric gap); this receipt is the honest M4c outcome for this branch"
        },
