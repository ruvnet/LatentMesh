//! Run-2 M4d — frozen-probe evaluation of the DEPLOY-MATCHED task-loss MLP
//! adapter (ADR-024 § "Registered contingency — M4d, train/deploy
//! configuration match"), run2_m4c_probe lineage.
//!
...
//! are `common::m3`'s SHARED code paths (the frozen protocol exists in
//! exactly one place and cannot silently diverge between rungs). The ONLY
//! change from M4c's probe: the artifact is M4d's (same architecture, same
//! layout, same hand-rolled apply). M4d changed TRAINING only — ADR-028
//! lists the deployment transform as evolvable and this protocol as
//! protected. ONE probe invocation, no second draw.
//!
//! REPORTING ADDITION (statistic, not protocol): every sign-test line also
//! carries the one-sided **mid-p McNemar** value on the same collected pairs
//! (`common::mid_p_one_sided`; ADR-030 / `docs/research/031` §2.4). It is
//! disclosed alongside the exact p and gates NOTHING — `gate_pass` and every
//! recorded verdict remain on the frozen exact sign test.
//!
//! ORDERING GATES (refused before any model loads):
//!   1. artifact hash == the M4d training receipt's (written before this
...
//!      cargo run --release --features cuda --example run2_m4d_probe
...
const TRANSFER_RECEIPT: &str = "receipts/run2-m4d-transfer-receipt-cellL18toL14.json";
...
fn main() -> anyhow::Result<()> {
    ...
    let expected_hash = train_receipt["artifact"]["content_hash_sha256"]
        ...
        .to_string();
    let artifact = crate_path("receipts/run2-m4d-mlp-deploymatch-cellL18toL14.f32bin");
    ...
    let golden = crate_path("receipts/run2-m4d-golden-mlp-deploymatch-cellL18toL14.json");
    let (golden_n, golden_max_rel, golden_seed) =
        common::mlp::verify_against_golden(&transform, &golden, GOLDEN_REL_TOL)?;
    println!(
        "M4d artifact ({}): hand-rolled apply verified against {golden_n} trained-network \
    ...
    let dir = common::run_dir("run2-m4d");
