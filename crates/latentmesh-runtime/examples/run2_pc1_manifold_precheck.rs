//! Run-2 **PC1 manifold pre-check** — the geometry of the positive control's
//! payload, on the record BEFORE the draw.
//!
//! ADR-024 § "PC1 PRE-REGISTRATION (2026-08-29)"; `docs/research/045` §5's
//! mandatory gates ("item-invariance/entropy check (`docs/research/036`'s
//! triple)"), run on the payload `run2_pc1_capture` wrote.
//!
//! **The expected result is trivial, and that is the point.** PC1's payload is
//! not an adapter's output — it *is* a real receiver residual-stream state
...
//! **pooled L14** state for the same item. PC1's payload is an **un-pooled
...
//!
//! **CPU-ONLY, ANNOTATES ONLY** (ADR-034 lane rule, same discipline as
//! `run2_manifold_precheck`): the only model-derived object touched is the
//! receiver's tied unembedding plus the final RMSNorm gain, on CPU. The
//! metric kit is `common/lens.rs` — the SAME code every prior pre-check used,
//! including the registered classification thresholds, reused rather than
//! re-implemented.
//!
//! **FIREWALL (`docs/research/045` §3, verbatim in the receipt):** a PASS on
//! PC1 proves **liveness only, never transfer**.
//!
//! Run (CPU, default features):
//!   cargo run --release --example run2_pc1_manifold_precheck
...
fn main() -> anyhow::Result<()> {
    ...
    let cap: serde_json::Value =
        serde_json::from_slice(&std::fs::read(crate_path(CAPTURE_RECEIPT)).map_err(|e| {
            anyhow::anyhow!(
                "capture receipt {CAPTURE_RECEIPT} unreadable ({e}) — run run2_pc1_capture first"
            )
        })?)?;
    anyhow::ensure!(
        cap["gates"]["receiver_L19_dump_sha256_verified"]["pass"].as_bool() == Some(true),
        "the capture receipt does not record a verified dump sha256"
    );
    anyhow::ensure!(
        cap["gates"]["s1a_item_set_reproduced"]["pass"].as_bool() == Some(true),
        "the capture receipt does not record the S1a item set"
    );
    anyhow::ensure!(
    ...
    let labels = [
        "pc1-payload-receiver-L19-lasttoken-goldteacherforced",
    ...
    for (k, label) in labels.iter().enumerate() {
        ...
        table.push(serde_json::json!({
            "label": label,
            "family": if k == 0 { "pc1-identity-no-adapter" } else { "reference-natural-receiver-state" },
    ...
    let receipt = serde_json::json!({
        "stage": "run2-PC1-manifold-precheck",
        ...
        "env": {
            "evidence_label": "deterministic CPU analysis over the sha-pinned PC1 payload — no probe draw, annotates only",
        ...
        "liveness_not_transfer": "A PASS on PC1 proves LIVENESS ONLY, NEVER TRANSFER, and may never be cited as evidence for the transfer claim.",
        "payload_source": {
        ...
        "registered_expectation_stated_before_the_numbers": {
            ...
            "if_it_is_not": "a finding about the PIPELINE, not about PC1: either the registered manifold metric's reference is mis-specified for un-pooled/other-depth states, or the pre-check has been mis-reading the adapters it has classified. Reported, not waved through.",
        ...
        "registered_metric_caveat": {
            ...
            "why_it_understates_here": "PC1's payload is an UN-POOLED L19 state; the metric's reference is a POOLED L14 state. BOTH depth (19 vs 14) and pooling (single row vs span mean) differ. ADR-024 already records that a pooled natural state sits only ~0.667 cosine from a real un-pooled one, so this number is not a clean on/off-manifold reading for this payload.",
    ...
    common::write_receipt(
        &crate_path("receipts"),
        "run2-pc1-manifold-precheck-receipt.json",
        &receipt,
    )?;
    println!(
        "PC1 pre-check: payload classified {payload_verdict} (expected on-manifold-item-varying; \
